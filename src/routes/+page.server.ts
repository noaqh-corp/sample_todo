...
const titleSchema = TodoCreateSchema.pick({ title: true }).extend({
...
const renameSchema = idSchema.merge(titleSchema);
...
export const actions = {
  create: async ({ request, locals }) => {
    ...
    const parsed = titleSchema.safeParse(Object.fromEntries(await request.formData()));
  ...
  rename: async ({ request, locals }) => {
    if (!locals.session) return fail(401, { error: "ログインが必要です" });
    const parsed = renameSchema.safeParse(Object.fromEntries(await request.formData()));
    if (!parsed.success) return fail(400, { error: "IDとタイトルを指定してください" });
